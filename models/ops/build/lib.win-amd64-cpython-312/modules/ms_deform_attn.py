...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, cross_domain=False, use_grl=False):
        """
        ...
        if cross_domain:
            ...
            self.attention_weights = nn.Linear(unit_dim, n_heads * n_levels * n_points)
            self.sampling_offsets = nn.Linear(unit_dim, n_heads * n_levels * n_points)
            self.value_proj = nn.Linear(unit_dim, unit_dim*2)
            self.output_proj = nn.Linear(unit_dim*2, unit_dim)

            # self.attention_weights_zd = nn.Linear(unit_dim*1, n_heads * n_levels * n_points)
            self.attention_weights_zd = nn.Linear(unit_dim*2, n_heads * n_levels * n_points)
            ...
            if use_grl:
                ...
                self.top_grl = nn.Identity()
                self.bot_grl = nn.Identity()
                self.top_grl2 = nn.Identity()
                self.bot_grl2 = nn.Identity()
        else:
    ...
    def _reset_parameters(self):
        ...
        grid_init = (grid_init / grid_init.abs().max(-1, keepdim=True)[0]).view(self.n_heads, 1, 1, 1).repeat(1, self.n_levels, self.n_points, 1)
    ...
    def cd_forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        #step 1: sample points,for ms deformable attention
        # N, Len_q, n_heads, n_levels, n_points
        sample_query = query[..., :self.unit_dim]
        sampling_offsets = self.sampling_offsets(sample_query).view(N, Len_q, self.n_heads, self.n_levels, self.n_points, 1)
        ...
        top_val = input_flatten[..., :self.unit_dim]
        top_val = self.value_proj(top_val) #(2, 18669, 256)
        if input_padding_mask is not None: # (2, 18669) 这段代码的主要功能是处理输入数据中的填充部分。具体来说，它通过 masked_fill 操作将 top_val 中对应于填充位置的值设置为 0.0，从而确保填充部分不会对后续的计算产生影响。
            top_val = top_val.masked_fill(input_padding_mask[..., None], float(0))
        top_val = top_val.view(N, Len_in, self.n_heads, self.unit_dim*2 // self.n_heads)

        # step 2.2 top branch query
        top_query = query[..., :self.unit_dim]
        ...
        bot_val = torch.cat((self.bot_grl(input_flatten[..., :self.unit_dim]), input_flatten[..., self.unit_dim:]), dim=-1)
        ...
        bot_query = torch.cat((self.bot_grl2(query[..., :self.unit_dim]), query[..., self.unit_dim:]), dim=-1)
