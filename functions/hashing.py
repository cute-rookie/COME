...
def prepare_db_dataset(config):
    ...
    n_labeled_num = config['sample']['labeled_sample_num']
    db_dataset = configs.dataset(config, filename=f'database_{n_labeled_num}.txt', transform_mode='test')
...
def get_hamm_dist(codes, centroids, margin=0, normalize=False):
    with torch.no_grad():
        ...
        dist = 0.5 * (nbit - torch.matmul(codes, centroids.t()))
        ...
        if margin == 0:
            ...
            dist_margin = 0.5 * (nbit - torch.matmul(codes_clone, centroids.t()))
            if normalize:
...
def calculate_recall(db_codes, db_labels, test_codes, test_labels, R, threshold=0.):
    # Clone in case changing value of the original codes
    db_codes = db_codes.clone()
    test_codes = test_codes.clone()
    ...
    # Calculate Recall
    recall_list = []
    for i in range(dist.shape[0]):  # For each query
        query_label = test_labels[i, :]  # Get query label
        query_label[query_label == 0] = -1  # Convert 0 to -1 for matching

        # Total relevant images in the database
        total_relevant = np.sum(np.sum(np.equal(db_labels, query_label), 1) > 0)

        # Retrieved relevant images in top R results
        retrieved_ids = topk_ids[i, :]  # Top R retrieved IDs
        retrieved_labels = db_labels[retrieved_ids, :]  # Labels of retrieved images
        retrieved_relevant = np.sum(np.sum(np.equal(retrieved_labels, query_label), 1) > 0)

        # Calculate Recall for this query
        if total_relevant > 0:
            recall = retrieved_relevant / total_relevant
        else:
            recall = 0  # If no relevant images in the database, recall is 0

        recall_list.append(recall)

    # Return average Recall
    return np.mean(np.array(recall_list))
...
def calculate_fairness(config,db_codes, db_labels,
    ...
    for i in range(dist.shape[0]): # dist.shape：[2000, 21708]
        ...
        if config['dataset'].startswith('utkface'):
            ...
            age = np.array(db_dataset.age_list)
            ethnicity = np.array(db_dataset.ethnicity_list)
            ta = np.array(eval(config['dataset_kwargs']['target_attribute']))
            sa = np.array(eval(config['dataset_kwargs']['sensitive_attribute']))
        elif config['dataset'].startswith('celeba'):
            att = np.array(db_dataset.att)
            ta = att[:,config['dataset_kwargs']['target_attribute']]
            sa = att[:,config['dataset_kwargs']['sensitive_attribute']]
        # if config['dataset'] == 'utkface_multicls':
        #     ta = ethnicity
        #     sa = age
