...
import configs
from functions.hashing import calculate_mAP, calculate_odds,calculate_fairness
...
def main(config):
    ...
    # res['odds'] = calculate_odds(config, db_out['codes'], db_out['labels'],
    #                            test_out['codes'], test_out['labels'],
    #                            loss_param['R'], loss_param['map_threshold'])
    ...
    print(f'equalized odds1: {res["odds1"]:.4f}')
    print(f'equalized odds2: {res["odds2"]:.4f}')
    #print(f'equalized odds3: {res["odds3"]:.4f}')
    print(f'equalized opportunities: {res["opportunities"]:.4f}')
    print(f'demographic parities: {res["dp"]:.4f}')
    #print(f'predictive parities: {res["pp"]:.4f}')
