...
def main(file_name=None):
    ...
    # parser.add_argument('--ds', default='utkface_multicls2', choices=['utkface','utkface_multicls','celeba','utkface_multicls2'], help='dataset')
    ...
    parser.add_argument('--ta', default='ethnicity', choices=['age', 'gender', 'ethnicity'], help='target attribute')
    parser.add_argument('--sa', default='age', choices=['age', 'ethnicity'], help='sensitive attribute')
    ...
    parser.add_argument('--LLM_for_image_type', default=1, help='choose the type of LLM to generate samples')
    ...
    parser.add_argument('--high_confidence_threshold', default=0.9, type=float, help='the confidence threshold for high confidence samples in LLM annotation')
    ...
    #           f'{config["optim"]}_'
    #           f'{config["ce"]}')
    ori_ce = config['loss_params']['ce']
    ori_final_feature = config['loss_params']['final_feature']
    ori_attributes_feature = config['loss_params']['attributes_feature']
    ori_lvl = config['loss_params']['lvl']
    for n_sample in [600]:
        for aug_diff in [True]:
            for annotation_unlabeled in [True]:
                for bit in [256]:
                    for topr in [5000]:
                        for step in range(7):
                            config['loss_params']['ce'] = ori_ce
                            config['loss_params']['final_feature'] = ori_final_feature
                            config['loss_params']['attributes_feature'] = ori_attributes_feature
                            config['loss_params']['lvl'] = ori_lvl
                            if step == 0:
                                config['loss_params']['final_feature'] = 0
                            elif step == 1:
                                config['loss_params']['attributes_feature'] = 0
                            elif step == 2:
                                config['loss_params']['lvl'] = 0
                            elif step == 3:
                                config['loss_params']['final_feature'] = 0
                                config['loss_params']['attributes_feature'] = 0
                            elif step == 4:
                            ...
                            #     config['loss_params']['attributes_feature'] = 0
                            #     config['loss_params']['lvl'] = 0
                            config['arch_kwargs']['hidden_dim'] = bit
                            config['arch_kwargs']['nbit'] = bit
                            # if ([n_sample, aug_diff, annotation_unlabeled, low_confidence_threshold, high_confidence_threshold] in
                            #         [[600, True, True, 0.2, 0.8], [600, True, True, 0.2, 0.9], [600, True, True, 0.3, 0.7], [600, True, False, 0.2, 0.8]]):
                            #     continue
                            # config['seed'] = s
                            config['sample']['labeled_sample_num'] = n_sample
                            config['aug_for_labeled_sample'] = aug_diff
                            config['annotation_unlabeled_data'] = annotation_unlabeled
                            config['R'] = topr
                            # config['low_confidence_threshold'] = low_confidence_threshold
                            # config['high_confidence_threshold'] = high_confidence_threshold
                            ######## logdir ############
                            # recorde the time
